/* ************************************************************************** */
/*                                                                            */
/*                                                        :::      ::::::::   */
/*   main.c                                             :+:      :+:    :+:   */
/*                                                    +:+ +:+         +:+     */
/*   By: del-khay <del-khay@student.42.fr>          +#+  +:+       +#+        */
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2023/03/21 23:00:10 by del-khay          #+#    #+#             */
/*   Updated: 2023/04/07 05:22:54 by del-khay         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "../Cub3D.h"

int	min(int a, int b)
{
	if (a < b)
		return (a);
	return (b);
}

void init_texture(t_mlx *mlx, t_components *comp)
{
	(void)comp;
	mlx->_t = malloc(sizeof(t_data) * 5);
	mlx->_s = malloc(sizeof(t_side) * RAYNUMBER);
	mlx->_t[NO].img = mlx_xpm_file_to_image(mlx->p_mlx, comp->elements.n_texture,
			&mlx->_t[NO].width, &mlx->_t[NO].height);
	mlx->_t[SO].img = mlx_xpm_file_to_image(mlx->p_mlx, comp->elements.s_texture,
			&mlx->_t[SO].width, &mlx->_t[SO].height);
	mlx->_t[WE].img = mlx_xpm_file_to_image(mlx->p_mlx, comp->elements.w_texture,
			&mlx->_t[WE].width, &mlx->_t[WE].height);
	mlx->_t[EA].img = mlx_xpm_file_to_image(mlx->p_mlx, comp->elements.e_texture,
			&mlx->_t[EA].width, &mlx->_t[EA].height);
	mlx->_t[DR].img = mlx_xpm_file_to_image(mlx->p_mlx, "testxpm/green.xpm",
			&mlx->_t[DR].width, &mlx->_t[DR].height);
	if (mlx->_t[NO].img == NULL || mlx->_t[SO].img == NULL || mlx->_t[WE].img == NULL
			|| mlx->_t[EA].img == NULL || mlx->_t[DR].img == NULL)
	{
		printf("Error reading texture files \n");
		exit(0);
	}
	mlx->_t[NO].addr = mlx_get_data_addr(mlx->_t[NO].img, &mlx->_t[NO].bits_per_pixel,
			&mlx->_t[NO].line_length, &mlx->_t[NO].endian);	
	mlx->_t[SO].addr = mlx_get_data_addr(mlx->_t[SO].img, &mlx->_t[SO].bits_per_pixel,
			&mlx->_t[SO].line_length, &mlx->_t[SO].endian);
	
	mlx->_t[WE].addr = mlx_get_data_addr(mlx->_t[WE].img, &mlx->_t[WE].bits_per_pixel,
			&mlx->_t[WE].line_length, &mlx->_t[WE].endian);
	mlx->_t[EA].addr = mlx_get_data_addr(mlx->_t[EA].img, &mlx->_t[EA].bits_per_pixel,
			&mlx->_t[EA].line_length, &mlx->_t[EA].endian);
	mlx->_t[DR].addr = mlx_get_data_addr(mlx->_t[DR].img, &mlx->_t[DR].bits_per_pixel,
			&mlx->_t[DR].line_length, &mlx->_t[DR].endian);
}

void init_map(t_mlx *mlx, t_components *comp)
{
	(void)comp;
	mlx->_m.map = comp->map;
	if (mlx->_m.map == NULL)
	{
		printf("Error reading map \n");
		exit(0);
	}
	mlx->_m.map_width = comp->map_w;
	mlx->_m.map_height = comp->map_h;
	mlx->_m.map_scale = min(SCREEN_WIDTH / mlx->_m.map_width, SCREEN_HEIGHT
			/ mlx->_m.map_height);
	if (mlx->_m.map_scale < 0)
	{
		printf("Error map is too big \n");
		exit(0);	
	};
	
	mlx->_m.display_map = 1;
	mlx->_m.img_xpos = 100 - mlx->_p.player_x;
	mlx->_m.img_ypos = 100 - mlx->_p.player_y;
}

void init_player(t_mlx *mlx, t_components *comp)
{
	(void)comp;
	mlx->_p.player_x = comp->player_x * mlx->_m.map_scale;
	mlx->_p.player_y = comp->player_y * mlx->_m.map_scale;
	mlx->_p.player_angle = M_PI / 2;
	mlx->_p.pmovment_speed = 4;
	mlx->_p.rotation_speed = 6 * (M_PI / 180);
	
}

void init_loadscreen(t_mlx *mlx)
{
	int i ,j;
	mlx->load_screen[0].img = mlx_xpm_file_to_image(mlx->p_mlx, "load/1.xpm",
			&i, &j); 
	mlx->load_screen[1].img = mlx_xpm_file_to_image(mlx->p_mlx, "load/2.xpm",
			&i, &j);
	mlx->load_screen[2].img = mlx_xpm_file_to_image(mlx->p_mlx, "load/3.xpm",
			&i, &j);
	mlx->load_screen[3].img = mlx_xpm_file_to_image(mlx->p_mlx, "load/4.xpm",
			&i, &j);
	mlx->load_screen[4].img = mlx_xpm_file_to_image(mlx->p_mlx, "load/5.xpm",
			&i, &j);
	mlx->load_screen[5].img = mlx_xpm_file_to_image(mlx->p_mlx, "load/6.xpm",
			&i, &j);
	mlx->load_screen[6].img = mlx_xpm_file_to_image(mlx->p_mlx, "load/7.xpm",
			&i, &j);
	mlx->load_screen[7].img = mlx_xpm_file_to_image(mlx->p_mlx, "load/8.xpm",
			&i, &j);
	mlx->load_screen[8].img = mlx_xpm_file_to_image(mlx->p_mlx, "load/9.xpm",
			&i, &j);
	mlx->load_screen[9].img = mlx_xpm_file_to_image(mlx->p_mlx, "load/10.xpm",
			&i, &j);
	mlx->load_screen[10].img = mlx_xpm_file_to_image(mlx->p_mlx, "load/11.xpm",
			&i, &j);
	mlx->load_screen[11].img = mlx_xpm_file_to_image(mlx->p_mlx, "load/12.xpm",
			&i, &j);
	mlx->load_screen[12].img = mlx_xpm_file_to_image(mlx->p_mlx, "load/13.xpm",
			&i, &j);
	mlx->load_screen[13].img = mlx_xpm_file_to_image(mlx->p_mlx, "load/14.xpm",
			&i, &j);
	mlx->load_screen[14].img = mlx_xpm_file_to_image(mlx->p_mlx, "load/15.xpm",
			&i, &j);
}

void init_gun(t_mlx *mlx)
{
	int i, j;
	
	mlx->gun_frames[0].img = mlx_xpm_file_to_image(mlx->p_mlx, "gunxpm/1.xpm",
			&i, &j);
	mlx->gun_frames[1].img = mlx_xpm_file_to_image(mlx->p_mlx, "gunxpm/2.xpm",
			&i, &j);
	mlx->gun_frames[2].img = mlx_xpm_file_to_image(mlx->p_mlx, "gunxpm/3.xpm",
			&i, &j);
	mlx->gun_frames[3].img = mlx_xpm_file_to_image(mlx->p_mlx, "gunxpm/4.xpm",
			&i, &j);
	mlx->gun_frames[4].img = mlx_xpm_file_to_image(mlx->p_mlx, "gunxpm/5.xpm",
			&i, &j);	
	mlx->gun_frames[5].img = mlx_xpm_file_to_image(mlx->p_mlx, "gunxpm/6.xpm",
			&i, &j);	
	mlx->gun_frames[6].img = mlx_xpm_file_to_image(mlx->p_mlx, "gunxpm/7.xpm",
			&i, &j);
	mlx->gun_img = mlx->gun_frames[0].img;
}

void	init_data(t_mlx *mlx, t_components *comp)
{
	int i;

	i = 0;
	mlx->on_change = 0;
	mlx->shoot = 0;
	init_texture(mlx, comp);
	init_map(mlx, comp);
	init_player(mlx, comp);
	init_loadscreen(mlx);
	init_gun(mlx);
	mlx->distances = malloc(sizeof(float) * RAYNUMBER);
	mlx->_d = calloc(RAYNUMBER ,sizeof(t_door));
	mlx->fov = 60 * (M_PI / 180);
	mlx->sreen_dist = (SCREEN_WIDTH / 2) / tan(mlx->fov / 2);
	mlx->mouse = 0;
	mlx->_mo.mouse_x = SCREEN_WIDTH / 2;
	mlx->_mo.mouse_y = SCREEN_HEIGHT / 2;
	mlx->_mo.mouse_angle = 0;
	mlx->_mo.y_offset = 0;
	while (i < 8)
	{
		mlx->control_keys[i] = RELEASED;
		i++;
	}
}

void	put_intro(t_mlx *mlx)
{
	int	i;
	int	j;
	// int frame;

	i = 0;
	j = 0;
	// frame = 0;
	mlx->win = mlx_new_window(mlx->p_mlx, SCREEN_WIDTH, SCREEN_HEIGHT, "Cub3D");
	mlx->_p.icon_img = mlx_xpm_file_to_image(mlx->p_mlx, "./testxpm/player.xpm", &i,
			&j);
	mlx->_p.gun_img = mlx_xpm_file_to_image(mlx->p_mlx, "./testxpm/gun.xpm", &i,
			&j);
	if (mlx->_p.icon_img == NULL || mlx->_p.gun_img == NULL)
	{
		printf("Error reading player icon \n");
		exit(0);
	}
	// while (!mlx->on_change)
	// {
	// 	if (frame == 15)
	// 		frame = 0;
	// 	mlx_put_image_to_window(mlx->p_mlx, mlx->win, mlx->load_screen[frame], 0, 0);
	// 	// sleep(1);
	// 	printf("frame: %d \r", frame);
	// 	frame++;
	// }
	
	// mlx->img = mlx_xpm_file_to_image(mlx->p_mlx, "./src/intro.xpm", &i, &j);
	// if (mlx->img == NULL)
	// {
	// 	printf("Error reading intro file \n");
	// 	exit(0);
	// }
	// mlx_put_image_to_window(mlx->p_mlx, mlx->win, mlx->img, 0, 0);
	// mlx_destroy_image(mlx->p_mlx, mlx->img);
}

int mouse_controls(int x, int y, t_mlx *mlx)
{

	int mouse_step = 0;

	
	mlx_mouse_hide();
	if (x != mlx->_mo.mouse_x || y != mlx->_mo.mouse_y)
	{
		mlx->mouse = 1;
		mlx->_mo.y_offset = ((y - (SCREEN_HEIGHT / 2)) * -1);
		mouse_step = x - mlx->_mo.mouse_x;
		mlx->_mo.mouse_angle = atan(abs(mouse_step / 2) / mlx->sreen_dist);
		if(mouse_step < 0)
			mlx->mouse = -1;
		else if (mouse_step >= 0)
			mlx->mouse = 1;
		mlx->_mo.mouse_x = x;
		if (mlx->_mo.mouse_x > SCREEN_WIDTH || mlx->_mo.mouse_x < 0)
		{
			mlx->_mo.mouse_x = SCREEN_WIDTH / 2;
			mlx_mouse_move(mlx->win, SCREEN_WIDTH / 2, y);
		}	
		if (y > SCREEN_HEIGHT + 100)
		{
			mlx->_mo.mouse_y = SCREEN_HEIGHT;
			mlx_mouse_move(mlx->win,mlx->_mo.mouse_x, SCREEN_HEIGHT);
		}
		else if (y < 0 - 100)
		{
			mlx->_mo.mouse_y = 0;
			mlx_mouse_move(mlx->win, mlx->_mo.mouse_x, 0);
		}
		else
			mlx->_mo.mouse_y = y;

	}
	return (0);
}

int mouse_click(int button, int x, int y, t_mlx *mlx)
{
	(void)x;
	(void)y;
	(void)mlx;
	if (button == 1)
	{
		if (mlx->shoot == 0)
			mlx->shoot = 1;
	}
	return (0);
}

void *framer(void *p)
{
	t_mlx *mlx;

	mlx = (t_mlx *)p;
	int frame = 0;
	while (!mlx->on_change)
	{
		if (frame == 15)
			frame = 0;
		mlx->load_screen_img = mlx->load_screen[frame].img;
		usleep(100000);
		frame++;
	}
	return (NULL);
}


void *gun_framer(void *p)
{
	t_mlx *mlx;
	int frame = 0;

	mlx = (t_mlx *)p;
	while (1)
	{
		
		if (mlx->shoot && mlx->on_change)
		{
			while (frame < 7)
			{
				mlx->gun_img = mlx->gun_frames[frame].img;
				usleep(60000);
				frame++;
			}
			mlx->gun_img = mlx->gun_frames[0].img;
			frame = 0;
			mlx->shoot = 0;
		}
	}
}


int	main(int argc, char **argv)
{
	t_mlx			mlx;
	t_components	comp;
	pthread_t     gun_thread;
	if (argc != 2)
		return (printf(YEL "Usage: ./cub3d map.cub \n" RESET));
	initialize_comps(&comp);
	if (!read_file(argv[1], &comp) || !check_comps(&comp))
		return (0);
	set_player_cords(&comp);
	// print_data(&comp);
	// exit(0);
	mlx.p_mlx = mlx_init();
	init_data(&mlx, &comp);
	pthread_create(&mlx.thread, NULL, &framer, &mlx);
	pthread_create(&gun_thread, NULL, &gun_framer, &mlx);
	put_intro(&mlx);
	mlx_hook(mlx.win, 17, 0, (int (*)()) & exit_game, &mlx);
	mlx_loop_hook(mlx.p_mlx, &render, &mlx);
	mlx_hook(mlx.win, 2, 0, &keys_pressed, &mlx);
	mlx_hook(mlx.win, 3, 0, &key_released, &mlx);
	// mlx hook for mouse
	mlx_hook(mlx.win, 6, 0, &mouse_controls, &mlx);
	mlx_hook(mlx.win,5,0, &mouse_click, &mlx);
	mlx_loop(mlx.p_mlx);
}

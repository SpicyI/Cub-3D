...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2023/04/09 03:35:14 by del-khay          #+#    #+#             */
/*   Updated: 2023/04/10 00:37:47 by del-khay         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	set_start(t_mlx *mlx, t_inter s)
{
	...
	start = 0;
	if (s.hit_side == HOR)
		start = ((s.sp_hitptf[X] - p[X]) / mlx->_m.map_scale)
			* mlx->sp_img.width;
	else if (s.hit_side == VER)
		start = ((s.sp_hitptf[Y] - p[Y]) / mlx->_m.map_scale)
			* mlx->sp_img.width;
	return (start);
}
